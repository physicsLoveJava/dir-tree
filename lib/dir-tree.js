...
function removeEmptyDirs(tree, options){
  ...
  while(stack.length > 0){
    ...
    if(current.cvisit === 0){
      for(var i = 0; i < current.children.length; i ++){
        if(current.children[i].isDirectory){
          current.children[i].visit = 0;
          current.children[i].cvisit = 0;
        ...
        fs.readdir(dirPath, function(err, fileList){
          ...
          fileList.forEach(function(file){
            if(filterFileType(file, options)){
              promises.push(fileStat(path.join(dirPath, file)));
            }
          });
...
function _checkIfDirectory(filePath){
  return Q.Promise(function(resolve, reject){
    fileStat(filePath).then(function(data){
...
module.exports._checkIfDirectory = _checkIfDirectory;
